...
if __name__=='__main__':
    # Morro Bay coordinates: 35.3658, -120.8499
    slo_wave_location = surfpy.Location(35.3658, -120.8499, altitude=0.0, name='Morro Bay')
    slo_wave_location.depth = 30.0
    slo_wave_location.angle = 270.0
    slo_wave_location.slope = 0.02
    ...
    wave_grib_data = west_coast_wave_model.fetch_grib_datas(0, num_hours_to_forecast, slo_wave_location)
    raw_wave_data = west_coast_wave_model.parse_grib_datas(slo_wave_location, wave_grib_data)
    ...
    slo_wind_location = surfpy.Location(35.3658, -120.8499, altitude=0.0, name='Morro Bay')
    weather_data = surfpy.WeatherApi.fetch_hourly_forecast(slo_wind_location)
    surfpy.merge_wave_weather_data(data, weather_data)

    for dat in data:
        dat.solve_breaking_wave_heights(slo_wave_location)
        dat.change_units(surfpy.units.Units.english)
    json_data = surfpy.serialize(data)
    with open('slo_forecast.json', 'w') as outfile:
    ...
    plt.title('GFS Wave West Coast: ' + west_coast_wave_model.latest_model_time().strftime('%d/%m/%Y %Hz'))
    
    # Add a legend
    plt.legend(loc='best')
    
    # Save the plot to a file instead of displaying it
    plt.savefig('slo_wave_heights.png')
    print(f"Plot saved to slo_wave_heights.png")
    plt.close()